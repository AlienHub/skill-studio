...
- 自动发现本机已存在的主流 Agent skill 目录，同时保留用户手动配置的目录。
...
- 无配置文件时：扫描所有本机已存在的内置候选目录。
- 有配置文件时：先使用用户配置目录，再追加本机存在但配置中缺失的内置候选目录。
- 所有目录会经过规范化、去重和存在性检查。
- 当前版本暂未实现“永久忽略内置目录”。如果从 UI 删除某个内置目录，但该目录仍然存在，下次加载时可能会再次自动补齐。
